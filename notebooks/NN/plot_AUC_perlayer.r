...
# model="NN" # "NN" or "MARS"
# REGION="RovHer_BRCA1" # "RovHer_BRCA1"  or "BRCA1_DATA"
...
# for y_layer in "class" "clinvar"; do
...
# # Subset data to keep rows where the VAR_WIN column matches the specified VAR_WIN
# data <- data %>%
#   mutate(VAR_WIN = as.numeric(VAR_WIN)) %>%  # Convert VAR_WIN to numeric
#   filter(VAR_WIN == as.numeric(VAR_WIN)) %>%  # Filter by VAR_WIN
#   filter(EMBED_COL == EMBED_COL) %>%  # Filter by EMBED_COL
