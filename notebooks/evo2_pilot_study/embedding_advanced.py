...
region=input_name
delta_csv2=Path(f"{input_name}_{LAYER}_delta_rev.csv")
ref_csv2=Path(f"{input_name}_{LAYER}_ref_rev.csv")
var_csv2=Path(f"{input_name}_{LAYER}_var_rev.csv")
...
if VAR_WIN == "1":
    ...
    for idx, row in data.iterrows(): # idx is the index number of the row.
        ...
        # embedding_ref = evo2.get_embeddings(dataset_ref_rev,LAYER) # LAYER
        # dataset_var_rev = evo2.process_data([var_reverse])
        # embedding_var = evo2.get_embeddings(dataset_var_rev,LAYER)
